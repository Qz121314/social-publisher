...
function errorSuggestion(platform: string, stage?: string | null) {
  const name = platformName(platform)
  switch (stage) {
    case 'opening_browser': return '检查 iXBrowser 是否运行、环境是否可打开，然后重新执行。'
    case 'checking_login': return `打开对应 iX 环境，完成人工登录或 ${name} 安全验证后再执行。`
    case 'checking_identity': return `检查 Channel 配置的发布身份与当前 ${name} 登录身份是否一致。`
    case 'opening_composer': return `${name} 页面结构可能变化，可先人工打开平台确认发布入口。`
    case 'uploading_media':
    case 'waiting_media': return `检查媒体文件是否存在、格式是否受支持，以及 ${name} 上传入口是否正常。`
    case 'submitting':
    case 'verifying': return `不要直接重试。先在 ${name} 人工确认帖子是否已经发布。`
...
function AttemptTimeline({ attempt }: { attempt: PublishAttempt }) {
  ...
  return (
    <section className="v1-task-detail-section">
      ...
      {events.length === 0 ? (
        <div className="v1-task-empty-timeline">该 Attempt 没有逐阶段 Timeline 事件。</div>
      ) : (
        <div className="v1-task-timeline">
          {events.map((event) => (
            <div className="v1-task-timeline-row" key={event.id}>
              <time>{eventTime(event.created_at)}</time><span className="v1-task-timeline-dot" />
...
export default function TasksPage() {
  ...
  const load = async () => {
    try {
      setTasks(await api<PublishJob[]>('/api/tasks/publish-jobs?limit=100'))
      setError(null)
    } catch (nextError) {
  ...
  const confirmNotPublishedAndRetry = async (job: PublishJob) => {
    ...
    if (!window.confirm(`请确认你已经在 ${name} 检查过：该帖子确实没有发布。确认后系统会创建安全重试。`)) return
    setBusyAction('retry')
    try {
  ...
  return (
    <main className="v1-page">
      <PageHeader eyebrow="任务中心" title="多平台执行任务与运行历史" description="统一查看 Facebook / Instagram PublishJob、Attempt、Timeline、耗时和人工确认。" actions={<PhaseBadge>Phase 8</PhaseBadge>} />
      {error && <div className="notice">{error}</div>}
      <p className="v1-inline-note">`needs_review` 对所有平台统一生效：只要最终发布动作可能已经发生但无法确认结果，就禁止自动重试，必须先人工检查对应平台。</p>

      <section className="v1-panel">
        <div className="v1-toolbar">
          <div className="filter-row">{filters.map((item) => <button key={item.value} type="button" className={`compact-button ${filter === item.value ? 'worker-button' : ''}`} onClick={() => setFilter(item.value)}>{item.label}</button>)}</div>
        ...
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {filtered.length === 0 ? <tr><td colSpan={8}><div className="empty-state compact-empty"><strong>暂无匹配任务</strong><span>从发布中心创建任务后会显示在这里。</span></div></td></tr> : filtered.map((task) => {
                ...
                return (
                  <tr key={task.id} className="v1-task-row" onClick={() => setSelectedId(task.id)}>
                    ...
                    <td><strong>{task.platform === 'instagram' && channel?.target_name ? `@${channel.target_name}` : channel?.target_name || 'Unknown Channel'}</strong><br /><small>iX #{channel?.profile_id ?? '—'}</small></td>
                    <td>{platformName(task.platform)}</td>
                    ...
                    <td onClick={(event) => event.stopPropagation()}><div className="v1-plan-actions"><button type="button" className="compact-button" onClick={() => setSelectedId(task.id)}>详情</button>{canRun && <button type="button" className="compact-button" onClick={() => runAgain(task)} disabled={busyAction === 'run'}>{task.status === 'failed' ? '重新执行' : '立即执行'}</button>}</div></td>
                  </tr>
                )
              })}
            </tbody>
        ...
        <div className="v1-task-drawer-backdrop" onMouseDown={() => setSelectedId(null)}>
          <aside className="v1-task-drawer" onMouseDown={(event) => event.stopPropagation()}>
            <header className="v1-task-drawer-header">
              <div><span>任务 #{selected.id.slice(0, 8)}</span><h2>{selected.platform === 'instagram' && selectedChannel?.target_name ? `@${selectedChannel.target_name}` : selectedChannel?.target_name || 'PublishJob 详情'}</h2><p>iX #{selectedChannel?.profile_id ?? '—'} · {platformName(selected.platform)} · {shortText(selectedContent?.text, 70)}</p></div>
            ...
            <div className="v1-task-drawer-body">
              ...
              {selected.status === 'needs_review' && (
                <section className="v1-review-panel">
                  ...
                  <p>系统可能已经执行 {platformName(selected.platform)} 最终发布动作，但无法确认结果。直接重试可能造成重复帖子。</p>
                  ...
                  <div className="v1-review-actions">
                    {selectedChannel?.target_url && <button type="button" className="compact-button" onClick={() => window.open(selectedChannel.target_url, '_blank', 'noopener,noreferrer')}>打开 {platformName(selected.platform)}</button>}
              ...
              {selectedLatestAttempt?.error_message && selected.status !== 'needs_review' && (
                <section className="v1-task-error-panel"><strong>失败阶段：{stageLabel(selectedLatestAttempt.stage)}</strong><p>原因：{selectedLatestAttempt.error_message}</p><p>建议：{errorSuggestion(selected.platform, selectedLatestAttempt.stage)}</p></section>
              )}

              {[...selected.attempts].sort((a, b) => b.attempt_no - a.attempt_no).map((attempt) => <AttemptTimeline key={attempt.id} attempt={attempt} />)}
