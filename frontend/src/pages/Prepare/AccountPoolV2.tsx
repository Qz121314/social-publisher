...
type ProxyEndpoint = { id: number; host: string; port: number; status: string; enabled: boolean }
type Account = {
  id: number
  name: string
  platform: string
...
function snapshotName(raw: string) {
  try {
    const value = JSON.parse(raw) as { name?: string }
    return value.name || '账号'
  } catch {
    return '账号'
  }
}
...
export default function AccountPoolV2Page() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      try {
        const next = await api<BatchTask>(`/api/batch-tasks/${activeBatch.id}`)
        setActiveBatch(next)
        await load()
      } catch (error) {
        setMessage(error instanceof Error ? error.message : String(error))
  ...
  const startBatchLogin = async (mode: 'group' | 'selection') => {
    ...
    setMessage(null)
    try {
      const task = await api<BatchTask>('/api/batch-tasks/login', {
        method: 'POST',
  ...
  return (
    <main className="prepare-workspace social-account-workspace">
      <WorkspaceHeader
        title="账号池"
        description="先批量准备账号、Cookie、2FA、分组和固定 IP；选择分组后直接批量登录，iX 环境会按需创建并长期绑定。"
        actions={<><Button onClick={() => setGroupEditor({ mode: 'create' })}><PlusIcon />新建分组</Button><Button variant="primary" onClick={() => setImportOpen(true)}>批量导入账号</Button></>}
      />
      <PrepareNav />
          ...
          {activeBatch.jobs.some((job) => job.status !== 'succeeded') && (
            <div className="batch-login-job-list">
              {activeBatch.jobs.filter((job) => job.status !== 'succeeded').slice(0, 12).map((job) => (
                <div key={job.id}><strong>{snapshotName(job.account_snapshot_json)}</strong><span>{jobStageLabel(job.stage)}</span><small>{job.error_message || (job.status === 'queued' ? '等待可用执行槽位' : '处理中')}</small></div>
              ))}
            </div>
          )}
        </section>
      ...
      <section className="social-account-shell">
        ...
        <div className="account-list-pane">
          <header className="account-scope-header">
            <div><div className="account-scope-title-row"><h2>{scopeTitle}</h2><StatusChip tone="neutral">{visibleAccounts.length} 个账号</StatusChip></div><p>{currentGroup?.description || '选分组 → 执行动作 → 只处理异常。分组在任务创建时会冻结为具体账号快照。'}</p></div>
          ...
          <div className="account-status-strip account-pool-status-strip">
            <div><span>已登录</span><strong>{loggedInCount}</strong></div><div><span>需要处理</span><strong>{attentionCount}</strong></div><div><span>未分配 IP</span><strong>{noProxyCount}</strong></div><div><span>待创建 iX 环境</span><strong>{noRuntimeCount}</strong></div>
          </div>

          <div className="account-toolbar"><div className="environment-search account-search"><SearchIcon /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索账号、IP或环境…" /></div><span className="account-toolbar-note">批量登录只恢复需要登录的账号，健康 Session 不重新登录。</span></div>

          {selected.length > 0 && <div className="environment-selection-bar account-selection-bar"><strong>已选择 {selected.length} 个账号</strong><span>可直接对当前选择创建批量登录任务。</span><div><Button variant="primary" onClick={() => startBatchLogin('selection')} disabled={busy}>批量登录</Button><Button onClick={() => setMoveOpen(true)}>移动分组</Button><Button onClick={assignProxy} disabled={busy}>自动分配 IP</Button></div></div>}
