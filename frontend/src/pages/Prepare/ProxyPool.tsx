...
export default function ProxyPoolPage() {
  ...
  return (
    <main className="prepare-workspace resource-pool-workspace">
      <WorkspaceHeader
        title="IP池"
        description="集中准备 SOCKS5。先批量导入和分配，后续批量登录任务会自动把固定 IP 写入账号对应的 iX 环境。"
        actions={<Button variant="primary" onClick={() => setImportOpen(true)}>批量导入 SOCKS5</Button>}
      />
      <PrepareNav />
      ...
      <section className="resource-pool-shell">
        <div className="resource-pool-toolbar">
          ...
          <span>账号会固定绑定 IP；批量登录不会随机更换 SOCKS5。</span>
        </div>
        ...
        <div className="resource-pool-table" role="table" aria-label="SOCKS5 IP池">
          ...
          {visible.length === 0 ? (
            <EmptyState title="IP池为空" description="批量导入 SOCKS5 后，可在账号池中自动分配给账号。" />
          ) : visible.map((item) => {
